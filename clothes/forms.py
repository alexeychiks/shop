...
class LoginForm(AuthenticationForm):
    ...
    password = forms.CharField(
        label='Пароль', widget=forms.PasswordInput(attrs={'class': 'pole'}))
...
class RegisterUserForm(forms.ModelForm):
    username = forms.CharField(
        required=True, label='Логин', widget=forms.TextInput(attrs={'class': 'pole'}))
    email = forms.EmailField(
        required=True, label='Почта', widget=forms.EmailInput(attrs={'class': 'pole'}))
    password1 = forms.CharField(
        label='Пароль', widget=forms.PasswordInput(attrs={'class': 'pole'}))
    password2 = forms.CharField(
        label='Пароль (повторно)', widget=forms.PasswordInput(attrs={'class': 'pole'}))
